/* Draft War Room layout + draft-speed controls + tier shading.
  ...
  document.querySelectorAll('.wr-tier .tier-scroll').forEach(scroll=>{
    ...
    Array.from(scroll.children).forEach(el=>{
      if(el.classList.contains('tier-label')){
        ...
        currentTier=match?match[1]:null;
        if(currentTier)el.dataset.tier=currentTier;
      }else if(el.classList.contains('player-row')&&currentTier){
        el.dataset.visualTier=currentTier;
      }
    });
  });
}
